...
{if $fatals}
   ...
   {if $fatals.times_limit}<div class="error">You can send internal transfer {$settings.limit_transfer_period_times} per
      {$settings.limit_transfer_period_date} only.</div>{/if}

{else}
   {if $errors}
      {if $errors.turing_image}<div class="error">Invalid turing image</div>{/if}
      {if $errors.user_not_found}<div class="error">The recipient's username entered has not been found or has been suspended.
</div>{/if}
...
{if $errors.too_big_amount_plus_fee}<div class="error">You have no enough funds to complte the transaction. Total amount
   you should have to send {$frm.amount|fiat:$frm.ec} + fee is <b>{$errors.too_big_amount_plus_fee}</b>.</div>{/if}
...
<form method=post>
   ...
   <table cellspacing=0 cellpadding=2 border=0 class="form internal_transfer_confirm">
      ...
      <tr>
         <th>2FA Code:</th>
         <td><input type="text" name="tfa_code" class=inpts size=15> <input type="hidden" name="tfa_time" id="tfa_time">
         </td>
      </tr>
      {literal}
...
<form method=post>
   ...
   <table cellspacing=0 cellpadding=2 border=0>
      ...
      {foreach from=$ps item=p}
            ...
            {if $p.balance > 0}{$currency_sign}{$p.balance} of {$p.name}
               {if $p.hold > 0} / {$currency_sign}{$p.hold} on hold
               {/if}<br>
            {/if}
      ...
      <tr>
       ...
       <td><select name=ec class=inpts>
         ...
         {foreach from=$ps item=p}
            ...
            {if $p.available > 0}<option value={$p.id}>{$p.name}</option>
            {/if}
      ...
      *}
         ...
         <table cellspacing=0 cellpadding=2 border=0 width=200>
            ...
            <tr>
               ...
               <td><input type=text name=amount value="{if $frm.amount}{$frm.amount|escape:htmlall}{else}10.00{/if}"
